...
void StaticsController::start()
{
    if (m_enStatus.load()==State::Running) return;
    subscribeAlert();
    m_enStatus.store(State::Running);
    LogMsg(u"StaticsController started"_s);
    m_workThread = std::thread(&StaticsController::runAction, this);
}
void StaticsController::stop()
{
...
bool StaticsController::isRunning() const
{
    return m_enStatus.load() == State::Running;
}
void StaticsController::setInterval(int interval)
{
    m_interval = interval;
}
bool StaticsController::subscribeAlert()
{
    BitTorrent::Session *session = BitTorrent::SessionImpl::instance();
    if (!session) return false; // Check if session is valid
    if (!connect(session, &BitTorrent::Session::torrentAdded, this, &StaticsController::onTorrentAdded))
    {
        return false; // Check if connection was successful
    }
    if (!connect(session, &BitTorrent::Session::torrentFinished, this, &StaticsController::onTorrentFinished))
    {
        return false; // Check if connection was successful
    }
    if(!connect(session, &BitTorrent::Session::torrentRemoved, this, &StaticsController::onTorrentRemove))
    {
        return false; // Check if connection was successful
    }
    if (!connect(session, &BitTorrent::Session::torrentStarted, this, &StaticsController::onTorrentStart))
    {
        return false; // Check if connection was successful
    }
    if (!connect(session, &BitTorrent::Session::torrentStopped, this, &StaticsController::onTorrentStop))
    {
        return false; // Check if connection was successful
    }
    return true; // Return true if subscription was successful, false otherwise
}
bool StaticsController::unsubscribeAlert()
{
    // Unsubscribe from alerts from the session
    BitTorrent::Session *session = BitTorrent::SessionImpl::instance();
    if (!session) return false; // Check if session is valid
    bool bRet = true;
    if (!disconnect(session, &BitTorrent::Session::torrentAdded, this, &StaticsController::onTorrentAdded))
    {
        bRet = false; // Check if disconnection was successful
    }
    if (!disconnect(session, &BitTorrent::Session::torrentFinished, this, &StaticsController::onTorrentFinished))
    {
        bRet = false; // Check if disconnection was successful
    }
    if (!disconnect(session, &BitTorrent::Session::torrentRemoved, this, &StaticsController::onTorrentRemove))
    {
        bRet = false; // Check if disconnection was successful
    }
    if (!disconnect(session, &BitTorrent::Session::torrentStarted, this, &StaticsController::onTorrentStart))
    {
        bRet = false; // Check if disconnection was successful
    }
    if (!disconnect(session, &BitTorrent::Session::torrentStopped, this, &StaticsController::onTorrentStop))
    {
        bRet = false; // Check if disconnection was successful
    }
...
void StaticsController::runAction()
{
    auto startTime = std::chrono::system_clock::now();
    auto endTime = startTime + std::chrono::milliseconds(m_interval);
    // Start the work thread
    LogMsg(u"StaticsController work thread started"_s, Log::INFO);
    ...
    qDebug()<<u"StaticsController work thread stopped"_s;
}
QString StaticsController::getHashId(const BitTorrent::InfoHash &infoHash) const
{
...
void StaticsController::handleStaticsStop()
{
    LogMsg(u"StaticsController stopped"_s, Log::INFO);
    // Archive all torrents info into history
    DB::TblTorrentInfo& tblTorrentInfo = DbStaticsStorage::instance().getTorrentInfoTable();
    DB::TblTorrentHistory& tblTorrentHistory = DbStaticsStorage::instance().getTorrentHistoryTable();
    ...
    DB::TblPeerInfo& tblPeerInfo = DbStaticsStorage::instance().getPeerInfoTable();
    DB::TblPeerHistory& tblPeerHistory = DbStaticsStorage::instance().getPeerHistoryTable();
    DB::TblContributionHistory& tblContributionHistory = DbStaticsStorage::instance().getContributionHistoryTable();
...
void StaticsController::handleTorrentAdded(const BitTorrent::Torrent *torrent)
{
...
void StaticsController::handleTorrentFinished(const BitTorrent::Torrent* torrent)
{
    if (!torrent) return; // Check if torrent is valid
...
void StaticsController::handleTorrentRemove(const std::shared_ptr<BitTorrent::InfoHash::WrappedType> infoHash)
{
...
void StaticsController::handleTorrentStart(const BitTorrent::Torrent* torrent)
{
    if (!torrent) return; // Check if torrent is valid
...
void StaticsController::handleTorrentStop(const BitTorrent::Torrent* torrent)
{
    if (!torrent) return; // Check if torrent is valid
...
bool StaticsController::handleCollectData()
{
    ...
    // ...
    DbStaticsStorage &db = DbStaticsStorage::instance();
...
bool StaticsController::handleTorrentContributionRequested(const BitTorrent::Torrent* torrent)
{
...
void StaticsController::onTorrentAdded(BitTorrent::Torrent *torrent)
{
    if (!torrent) return; // Check if torrent is valid
    std::scoped_lock lock(this->m_mutex);
...
void StaticsController::onTorrentFinished(BitTorrent::Torrent *torrent)
{
    if (!torrent) return; // Check if torrent is valid
    std::scoped_lock lock(this->m_mutex);
    this->m_alertQueue.push({
...
void StaticsController::onTorrentRemove(const BitTorrent::InfoHash::WrappedType &torrentID)
{
    std::scoped_lock lock(this->m_mutex);
    this->m_alertQueue.push({
...
void StaticsController::onTorrentStart(BitTorrent::Torrent *torrent)
{
    if (!torrent) return; // Check if torrent is valid
    std::scoped_lock lock(this->m_mutex);
    this->m_alertQueue.push({
...
void StaticsController::onTorrentStop(BitTorrent::Torrent *torrent)
{
    if (!torrent) return; // Check if torrent is valid
    std::scoped_lock lock(this->m_mutex);
    this->m_alertQueue.push({
...
void StaticsController::onRequestTorrentContribution(const BitTorrent::Torrent *torrent)
{
    if (!torrent) return; // Check if torrent is valid
    std::scoped_lock lock(this->m_mutex);
    this->m_alertQueue.push({
